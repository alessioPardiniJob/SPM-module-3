...
LOG_FILE="log_detailed_strong_scalability_tasklevel.txt"
CSV_UNIFORM="strong_scalability_uniform_tasklevel.csv"
CSV_SKEWED="strong_scalability_skewed_tasklevel.csv"
...
for dataset_name in "uniform" "skewed"; do
    ...
    for T in "${THREADS_LIST[@]}"; do
        ...
        RUN_OUTPUT=$(srun -w "$NODE" --exclusive $EXEC \
            -nr $NR -ns $NS -seed $SEED -max-key $MAX_KEY -p $P  -chunk-size $BEST_CHUNK -t $T \
            $dataset_params)
