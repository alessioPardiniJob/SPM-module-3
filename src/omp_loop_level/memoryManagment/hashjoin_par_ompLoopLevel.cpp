...
// This function sets the OpenMP schedule according to the specified type and chunk size.
    ...
    PhaseTiming& T)
{
...

    {
        ...
        // Parallel join loop: each thread processes a subset of partitions independently.
        #pragma omp parallel num_threads(num_threads) \
            default(none) shared(Rpart, Spart, T, R, thread_results, thread_timings, p, std::cout)       

        {

            // every thread has its own local accumulators and timing variables to avoid contention and false sharing.
            ...
            // The for loop is parallelized with OpenMP, and each thread processes a subset of partitions. The schedule(runtime) directive allows us to control the scheduling strategy (static, dynamic, guided, or auto) at runtime, which can help optimize load balancing based on the characteristics of the data and the system.
            #pragma omp for schedule(runtime) \
                nowait // We use nowait to allow threads to proceed to the accumulation phase as soon as they finish their assigned partitions, without waiting for all threads to complete the loop.
            for (std::uint32_t pid = 0; pid < p; ++pid) {
                JoinResult local = join_one_partition_optimized(Rpart, Spart, pid, local_timing, thread_map);
                
                // evry thread accumulates its local results into its own slot in the thread_results array, which is aligned to avoid false sharing. This allows us to safely aggregate results without synchronization, as each thread writes to a separate cache line.
            ...
            // After processing its assigned partitions, each thread writes its local totals and timings into the shared thread_results and thread_timings arrays. Since each thread writes to a separate index corresponding to its thread ID, there
            thread_results[threadid].data = local_total;
            thread_timings[threadid].data = local_timing;
        } 
        // After the parallel region, all threads have completed their work, and we can safely aggregate the results from the thread_results array without any risk
    }

    // accumulate results from all threads into the final total. Since each thread has written its results to a separate index in the thread_results array, we can simply iterate over this array and sum up the join counts and checksums to get the final result. We also take the maximum build and probe times across all threads to report the overall timing for these phases.
