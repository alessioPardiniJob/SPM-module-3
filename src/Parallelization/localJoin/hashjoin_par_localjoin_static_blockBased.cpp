    ...
    PhaseTiming& T)
{
...

    {
        ...
        auto block_based = [&](int threadid) {
            std::uint32_t parts_per_thread = p / num_threads;
            std::uint32_t rem = p % num_threads;
            std::uint32_t lower = threadid * parts_per_thread + std::min(static_cast<std::uint32_t>(threadid), rem);
            std::uint32_t upper = lower + parts_per_thread + (threadid < static_cast<int>(rem) ? 1 : 0);
            ...
            for (std::uint32_t pid = lower; pid < upper; ++pid) {
                ...
                JoinResult local = join_one_partition(Rpart, Spart, pid, local_timing);
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < RUNS + 2; i++) {
        ...
        JoinResult r = partitioned_hash_join_sequential(R, S, P, nthreads, T);
