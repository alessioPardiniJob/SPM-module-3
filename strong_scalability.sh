...
LOG_FILE="log_detailed_strong_scalability.txt"
CSV_FILE="strong_scalability_results.csv"

# Construct the CSV header: each metric is paired with its corresponding standard deviation (_std) column
HEADER="Threads,"
...
# Iterate through the specified thread counts to perform scalability tests
for T in "${THREADS_LIST[@]}"; do
    echo -ne "${YELLOW}>> Starting Test with $T Thread... ${NC}"
    
    # Execute the binary on the designated compute node and capture the standard output
    ...
    # Extracts the 4th column (median) and the 12th column (standard deviation) based on the execution output format
    extract_pair() {
    ...
    # Segment the raw output to prevent label collisions between relation R and relation S
    ...
    # Extract comma-separated pairs of (median, standard deviation) for each execution phase
    ...
    # Append the extracted metrics as a new formatted row in the CSV file
    echo "$T,$RES_HIST_R,$RES_SCAT_R,$RES_HIST_S,$RES_SCAT_S,$RES_BUILD,$RES_PROBE,$RES_LOOP,$RES_TOTAL" >> $CSV_FILE
    
    # Isolate the total median time to display a brief summary in the console feedback
    TOTAL_VAL=$(echo $RES_TOTAL | cut -d',' -f1)
    echo -e "${GREEN}OK! (Total: ${TOTAL_VAL}ms)${NC}"
    
    # Append detailed execution logs to the designated log file for further inspection
