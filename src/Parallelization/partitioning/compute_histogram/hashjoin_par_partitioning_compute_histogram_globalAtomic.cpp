...
//
static std::vector<std::size_t> compute_histogram_atomic(const std::vector<Record>& rel, std::uint32_t p, std::uint32_t num_threads) {
    std::vector<std::size_t> final_hist(p, 0);
    
    // global atomic histogram: shared among threads, with atomic increments to avoid race conditions
    std::unique_ptr<std::atomic<std::size_t>[]> atomic_hist(new std::atomic<std::size_t>[p]{});
    ...
    auto block_based = [&](int threadid) {
        ...
        for (std::size_t i = lower; i < upper; ++i) {
            ...
            // Atomically increment the count for this partition
            atomic_hist[pid].fetch_add(1); 
        }
    };
    ...
    // Copy atomic counts to the final histogram vector
    for (std::uint32_t i = 0; i < p; ++i) {
        final_hist[i] = atomic_hist[i].load();
    }
...
static PartitionedRelation partition_relation(const std::vector<Record>& rel, std::uint32_t p, std::uint32_t num_threads, PhaseTiming::PartitionPhase& T_part) {
...

    {
        ...
        hist = compute_histogram_atomic(rel, p, num_threads);
    }
