...
static std::vector<Record> scatter_scalar_optimized(
    const std::vector<Record>& rel,
    std::uint32_t p,
    std::uint32_t num_threads,
    const std::vector<std::vector<std::size_t>>& thread_offsets) 
{
    // The output array that will hold the partitioned records. It has the same size as the input relation.
    std::vector<Record> out(rel.size());

    std::vector<std::thread> threads;
    threads.reserve(num_threads);

    // Each thread will have its own set of write cursors (offsets) for each partition, initialized from the thread_offsets computed in the prefix sum phase.
    auto worker = [&, num_threads](std::uint32_t threadid) {
        ...
        // Compute the range of records this thread will process in a block-based manner, similar to the histogram computation.
        std::size_t total_records = rel.size();
        std::size_t parts_per_thread = total_records / num_threads;
        std::size_t rem = total_records % num_threads;
        ...
        for (std::size_t i = lower; i < upper; ++i) {
            std::uint32_t pid = compute_partition_id(rel[i].key, p);
            // Write the record to the output array at the position indicated by my_next[pid], and then increment the cursor for that partition.
            out[my_next[pid]++] = rel[i];
        }
    };

    // Launch threads to perform the scatter in parallel
    for (std::uint32_t id = 0; id < num_threads; ++id) {
    ...
    // After all threads have completed, the output array 'out' contains the records rearranged so that all records of the same partition are contiguous in memory.
    return out;
}
...
static PartitionedRelation partition_relation(const std::vector<Record>& rel, std::uint32_t p, std::uint32_t num_threads, PhaseTiming::PartitionPhase& T_part) {
...

    {
        ...
        data = scatter_scalar_optimized(rel, p, num_threads, prefix_res.thread_offsets);
    }

    {
        ScopedTimer t(T_part.end);
        // Compute the end offsets for each partition based on the begin offsets and the counts from the local histograms.
        for (std::uint32_t i = 0; i < p - 1; i++) {
            end[i] = prefix_res.begin[i + 1];
        }
        end[p - 1] = rel.size(); // The end offset of the last partition is the total size of the relation, since it occupies the remaining space in the output array.
    }
...
int main(int argc, char** argv) {
    ...
    // Utility lambda to extract a vector of timings for a specific phase from the collected times.
