    ...
    const std::vector<std::vector<std::size_t>>& thread_offsets)
{
        ...
        default(none) shared(rel, out, p, thread_offsets)
    {
        ...
        for (std::size_t i = 0; i < rel.size(); ++i) {
            ...
            // Increment the buffer count for this partition. If the buffer is full, flush it to the output array and reset the buffer count.
        ...
        // After processing all records, there may be some remaining records in the local buffers that have not been flushed to the output array. We need to flush these remaining records for each partition.
        for (std::uint32_t pid = 0; pid < p; ++pid) {
...
static JoinResult join_one_partition_optimized(const PartitionedRelation& Rpart,
    ...
    if (countR.bucket_count() > needed_buckets) {
        // The map is larger than necessary. We perform a swap with a newly allocated empty map of the appropriate size.
        ska::flat_hash_map<std::uint64_t, std::uint32_t> fresh;
        fresh.reserve(current_r_size);
        countR.swap(fresh);
        // 'fresh' (holding the old memory) is destroyed here
    } else {
        // The map is already appropriately sized or smaller.
        // clear() is O(bucket_count()), but bucket_count() <= needed_buckets,
        // therefore it scales with the current partition size — acceptable.
        countR.clear();
        // reserve() here is useful only if the map is smaller than required
        // (first partition or after a swap on a very small partition)
...
// We define aligned structures to ensure that each thread's results and timings are stored in separate cache lines, minimizing false sharing and improving performance during the parallel join phase.
    ...
    PhaseTiming& T)
{
...

    {
        ...
        // Parallel join on partitions using OpenMP. Each thread processes a subset of the partitions independently, accumulating results in thread-local variables to avoid contention. The OpenMP 'for' loop with dynamic scheduling allows threads to pick up partitions as they finish, which can help with load balancing, especially if partition sizes are skewed.
        #pragma omp parallel num_threads(num_threads) \
            default(none) shared(Rpart, Spart, T, R, thread_results, thread_timings, p, std::cout)       

        {
            // Each thread gets its own thread ID to index into the thread_results and thread_timings arrays, ensuring that there are no data
            int threadid = omp_get_thread_num();

            // Each thread maintains local accumulators for the join results and timings to avoid contention on shared variables. These local variables are updated as the thread processes its assigned partitions, and at the end of the parallel region, each thread writes its results to its designated slot in the thread_results and thread_timings arrays.
            ...
            // OpenMP for loop to process each partition in parallel. The 'schedule(runtime)' directive allows the scheduling strategy to be determined at runtime based on the OpenMP environment variables, providing flexibility for different workloads and system configurations. The 'nowait' clause is used to avoid an implicit barrier at the end of the loop, allowing threads to proceed to save their results without waiting for others, which can improve performance when there is variability in partition sizes.
            #pragma omp for schedule(runtime) \
                nowait // We will handle the barrier manually after this loop to ensure that all threads have completed their work before we aggregate results.
            for (std::uint32_t pid = 0; pid < p; ++pid) {
                
                
                JoinResult local = join_one_partition_optimized(Rpart, Spart, pid, local_timing, thread_map);
                
                // After processing each partition, the thread updates its local accumulators with the results from that partition. This allows each thread to maintain a running total of the join count and checksums for all the partitions it has processed, without needing to synchronize with other threads until the end of the parallel region.
            ...
            // After all partitions have been processed, each thread writes its local results and timings to the shared thread_results and thread_timings arrays. Since each thread writes to a unique index corresponding to its thread ID, there
            thread_results[threadid].data = local_total;
            thread_timings[threadid].data = local_timing;
        } 
        // End of parallel region. At this point, all threads have completed their work and have stored their results in the thread_results and thread_timings arrays. We can now proceed to aggregate these results in the master thread.
    }
