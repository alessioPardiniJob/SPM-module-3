...
//
static std::vector<std::size_t> compute_histogram_privatized(const std::vector<Record>& rel, std::uint32_t p, std::uint32_t num_threads) {
    std::vector<std::size_t> final_hist(p, 0);
    ...
    // Block-based distribution strategy
    auto block_based = [&](int threadid) {
        ...
        for (std::size_t i = lower; i < upper; ++i) {
            ...
            // Increment local histogram count for this partition
            my_hist[pid]++; 
        }
    };
...
static std::vector<Record> scatter_partitioned_naive(
    ...
    
{
    ...
    // Atomic write cursors for each partition to avoid contention
    std::vector<std::atomic<std::size_t>> next(p);
    for (std::uint32_t i = 0; i < p; ++i) {
        next[i].store(begin[i], std::memory_order_relaxed);
    }
    ...
    auto worker = [&](int threadid) {
        ...
        // Scatter the assigned block of records into the output array using atomic write cursors
        for (std::size_t i = lower; i < upper; ++i) {
            std::uint32_t pid = compute_partition_id(rel[i].key, p);
            
            // Atomically fetch and increment the write cursor for this partition to get the index where this record should be written
            std::size_t write_idx = next[pid].fetch_add(1, std::memory_order_relaxed);
            
            // Write the record to the output array at the position indicated by the write cursor
            out[write_idx] = rel[i];
        }
    };

    // Spawn threads to perform the scatter in parallel
    for (std::uint32_t id = 0; id < num_threads; ++id) {
        threads.emplace_back([&, threadid = id]() { worker(threadid); });
    }
...
static PartitionedRelation partition_relation(const std::vector<Record>& rel, std::uint32_t p, std::uint32_t num_threads, PhaseTiming::PartitionPhase& T_part) {
...

    {
        ...
        hist = compute_histogram_privatized(rel, p, num_threads);
    }
...

    {
        ...
        data = scatter_partitioned_naive(rel, p, num_threads, begin);
    }
...
int main(int argc, char** argv) {
    ...
    // Utility to extract a vector of timings for a specific phase across all runs
